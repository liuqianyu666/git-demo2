...
import com.tjetc.common.ChatTypeVerificationUtil;
import com.tjetc.common.UserChannelRel;
import com.tjetc.entity.ChatMsgVO;
import com.tjetc.entity.ChatVO;
import com.tjetc.enums.MsgActionEnum;
            ...
            if (action.equals(MsgActionEnum.CONNECT.type)) {
                ...
                if (offLineMessageList != null) {
                    ...
                    for (int i = 0; i < offLineMessageList.size(); i++) {
                        //离线消息json转javabean
                        ChatVO chatVO = JSONUtil.toBean(offLineMessageList.get(i).toString(),ChatVO.class);
                        //将离线消息发送给当前用户
                ...
                //缓存失败，抛出异常
                if (!isCache) {
                    throw new RuntimeException("聊天内容缓存失败,聊天信息内容：" + message);
                }
            } else if (action.equals(MsgActionEnum.KEEPALIVE.type)) {
                //心跳类型的消息
                log.info("收到来自channel为[" + currentChannel + "]的心跳包");
            } else if (action.equals(MsgActionEnum.BREAK.type)){
                ChatVO chatVO = JSONUtil.toBean(message, ChatVO.class, true);
                UserChannelRel.remove(chatVO.getFromUserId());
            }
